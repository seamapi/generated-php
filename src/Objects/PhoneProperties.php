...
class PhoneProperties
{
    ...
    public static function from_json(mixed $json): PhoneProperties|null
    {
        ...
        return new self(
            ...
            battery: PhoneBattery::from_json($json->battery) ?? null,
            ...
            august_metadata: PhoneAugustMetadata::from_json($json->august_metadata) ?? null,
            avigilon_alta_metadata: PhoneAvigilonAltaMetadata::from_json($json->avigilon_alta_metadata) ?? null,
            schlage_metadata: PhoneSchlageMetadata::from_json($json->schlage_metadata) ?? null,
            smartthings_metadata: PhoneSmartthingsMetadata::from_json($json->smartthings_metadata) ?? null,
            lockly_metadata: PhoneLocklyMetadata::from_json($json->lockly_metadata) ?? null,
            nuki_metadata: PhoneNukiMetadata::from_json($json->nuki_metadata) ?? null,
            kwikset_metadata: PhoneKwiksetMetadata::from_json($json->kwikset_metadata) ?? null,
            salto_metadata: PhoneSaltoMetadata::from_json($json->salto_metadata) ?? null,
            genie_metadata: PhoneGenieMetadata::from_json($json->genie_metadata) ?? null,
            brivo_metadata: PhoneBrivoMetadata::from_json($json->brivo_metadata) ?? null,
            igloo_metadata: PhoneIglooMetadata::from_json($json->igloo_metadata) ?? null,
            noiseaware_metadata: PhoneNoiseawareMetadata::from_json($json->noiseaware_metadata) ?? null,
            minut_metadata: PhoneMinutMetadata::from_json($json->minut_metadata) ?? null,
            four_suites_metadata: PhoneFourSuitesMetadata::from_json($json->four_suites_metadata) ?? null,
            two_n_metadata: PhoneTwoNMetadata::from_json($json->two_n_metadata) ?? null,
            controlbyweb_metadata: PhoneControlbywebMetadata::from_json($json->controlbyweb_metadata) ?? null,
            ttlock_metadata: PhoneTtlockMetadata::from_json($json->ttlock_metadata) ?? null,
            seam_bridge_metadata: PhoneSeamBridgeMetadata::from_json($json->seam_bridge_metadata) ?? null,
            igloohome_metadata: PhoneIgloohomeMetadata::from_json($json->igloohome_metadata) ?? null,
            nest_metadata: PhoneNestMetadata::from_json($json->nest_metadata) ?? null,
            ecobee_metadata: PhoneEcobeeMetadata::from_json($json->ecobee_metadata) ?? null,
            hubitat_metadata: PhoneHubitatMetadata::from_json($json->hubitat_metadata) ?? null,
            dormakaba_oracode_metadata: PhoneDormakabaOracodeMetadata::from_json($json->dormakaba_oracode_metadata) ?? null,
            wyze_metadata: PhoneWyzeMetadata::from_json($json->wyze_metadata) ?? null,
          ...
          $json->code_constraints ?? []
            ...
            keypad_battery: PhoneKeypadBattery::from_json($json->keypad_battery) ?? null,
            ...
            current_climate_setting: PhoneCurrentClimateSetting::from_json($json->current_climate_setting) ?? null,
            default_climate_setting: PhoneDefaultClimateSetting::from_json($json->default_climate_setting) ?? null,
            is_climate_setting_schedule_active: $json->is_climate_setting_schedule_active ?? null,
            active_climate_setting_schedule: PhoneActiveClimateSettingSchedule::from_json($json->active_climate_setting_schedule) ?? null,
